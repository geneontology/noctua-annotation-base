  ...
  onValueChanges() {
    ...
    this.filteredOrganisms = this.filterForm.controls.organisms.valueChanges
      .pipe(
        ...
        map(organism => organism ? this.searchService.filterOrganisms(organism) : this.searchService.organisms.slice())
      )
    ...
    this.filteredStates = this.filterForm.controls.states.valueChanges
      .pipe(
        ...
        map(
          value => typeof value === 'string' ? value : value['name']),
        map(state => state ? this.searchService.filterStates(state) : this.searchService.states.slice())
      )
  }
